    ...
    <div @click.away="showAddPetModal = false" class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <div class="p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                @if(Session::has('fail'))
                <span class="bg-green-100 border border-red-400 text-red-500 px-4 py-2 rounded">{{Session::get('fail')}}</span>
                @endif
            </div>
            
            <form method="post" action="{{route('EditPet')}}">
                <input type="hidden" name="pet_id" id="" value="{{$pet->id}}">
                @csrf
                <div class="space-y-4">
                    <!-- Pet Name -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Pet Name</label>
                        <input x-model="newPet.name" type="text" name="pet_name" value="{{$pet->pet_name}}" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-petpurple">
                        @error('pet_name')
                    ...
                    <div>
                        ...
                        <select x-model="newPet.type" required name="pet_type" value="{{$pet->pet_type}}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-petpurple">
                    ...
                    <div>
                        ...
                        <div class="flex items-center">
                            <input x-model="newPet.age" type="number" name="age" value="{{$pet->age}}" min="0" required
                    ...
                    <div>
                        ...
                        <select x-model="newPet.healthStatus" required name="health_status" value="{{$pet->health_status}}"
